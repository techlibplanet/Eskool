...
class CircularsFragment : BaseFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        LocalBroadcastManager.getInstance(context!!).registerReceiver(downloadStateReceiver,
                IntentFilter(Constants.DOCUMENT_DOWNLOAD_STATUS_BROADCAST))
    }

    override fun onDestroy() {
        LocalBroadcastManager.getInstance(context!!).unregisterReceiver(downloadStateReceiver)
        super.onDestroy()
    }
